-- Per-day booking schedule support + Storage DELETE support for permanent media removal.
...
set weekly_schedule = jsonb_build_object(
  '0', jsonb_build_object(
    'enabled', 0 = any(open_days),
    'open_time', lpad(open_hour::text, 2, '0') || ':00',
    'close_time', case when close_hour >= 24 then '23:59' else lpad(close_hour::text, 2, '0') || ':00' end
  ),
  '1', jsonb_build_object(
    'enabled', 1 = any(open_days),
    'open_time', lpad(open_hour::text, 2, '0') || ':00',
    'close_time', case when close_hour >= 24 then '23:59' else lpad(close_hour::text, 2, '0') || ':00' end
  ),
  '2', jsonb_build_object(
    'enabled', 2 = any(open_days),
    'open_time', lpad(open_hour::text, 2, '0') || ':00',
    'close_time', case when close_hour >= 24 then '23:59' else lpad(close_hour::text, 2, '0') || ':00' end
  ),
  '3', jsonb_build_object(
    'enabled', 3 = any(open_days),
    'open_time', lpad(open_hour::text, 2, '0') || ':00',
    'close_time', case when close_hour >= 24 then '23:59' else lpad(close_hour::text, 2, '0') || ':00' end
  ),
  '4', jsonb_build_object(
    'enabled', 4 = any(open_days),
    'open_time', lpad(open_hour::text, 2, '0') || ':00',
    'close_time', case when close_hour >= 24 then '23:59' else lpad(close_hour::text, 2, '0') || ':00' end
  ),
  '5', jsonb_build_object(
    'enabled', 5 = any(open_days),
    'open_time', lpad(open_hour::text, 2, '0') || ':00',
    'close_time', case when close_hour >= 24 then '23:59' else lpad(close_hour::text, 2, '0') || ':00' end
  ),
  '6', jsonb_build_object(
    'enabled', 6 = any(open_days),
    'open_time', lpad(open_hour::text, 2, '0') || ':00',
    'close_time', case when close_hour >= 24 then '23:59' else lpad(close_hour::text, 2, '0') || ':00' end
  )
)
...
alter table public.booking_settings
  ...
  '{
    "0":{"enabled":true,"open_time":"11:00","close_time":"18:00"},
    "1":{"enabled":false,"open_time":"11:00","close_time":"18:00"},
    "2":{"enabled":false,"open_time":"11:00","close_time":"18:00"},
    "3":{"enabled":false,"open_time":"11:00","close_time":"18:00"},
    "4":{"enabled":false,"open_time":"11:00","close_time":"18:00"},
    "5":{"enabled":false,"open_time":"11:00","close_time":"18:00"},
    "6":{"enabled":false,"open_time":"11:00","close_time":"18:00"}
  }'::jsonb,
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'booking_settings_weekly_schedule_object'
      and conrelid = 'public.booking_settings'::regclass
  ) then
    alter table public.booking_settings
      add constraint booking_settings_weekly_schedule_object
...
begin
  ...
  v_cursor := p_date + v_open_time;
  v_last_start := (p_date + v_close_time) - make_interval(mins => v_service.duration_minutes);
...
drop policy if exists barber_media_admin_select on storage.objects;
create policy barber_media_admin_select on storage.objects
